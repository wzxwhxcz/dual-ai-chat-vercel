...
import { FileText, Eye, Code, Copy, Check, Maximize, Minimize, Undo2, Redo2, History } from 'lucide-react';
import { marked } from 'marked';
import DOMPurify from 'dompurify';
import NotepadHistory from './NotepadHistory';
import { useNotepadHistory } from '../hooks/useNotepadHistory';
...
interface NotepadProps {
  ...
  canUndo: boolean;
  canRedo: boolean;
  onContentChange?: (content: string, author: MessageSender | null) => void;
}
...
const Notepad: React.FC<NotepadProps> = ({
  ...
  const handleSelectVersion = (versionId: string) => {
    ...
    if (version && onContentChange) {
      onContentChange(version.content, version.author);
    }
  ...
  const handleShowHistory = () => {
    // 保存当前版本到历史
    if (content && (!versions.length || versions[versions.length - 1].content !== content)) {
      addVersion(content, lastUpdatedBy || null, '查看历史前的版本');
    }
    setShowHistory(true);
  };
